...
export default {
  ...
  mounted () {
    ...
    this.$axios.$get(`${this.$config.publicApiBaseUrl}/blog/active?page=1&limit=9`)
      .then((res) => {
  ...
  methods: {
    ...
    onSearchButtonClick () {
      this.isLoadingNews = true
      this.$axios.$get(`${this.$config.publicApiBaseUrl}/blog/active?page=1&limit=9&title=${this.searchKeyword}`)
        .then((res) => {
          this.news = res.data.result
          this.pagination = res.data.pagination
          this.current = this.pagination.page
